...
def _chunk(doc_id, name, text, page=7, section="Termination"):
    return {
        "score": 0.9, "chunk_id": f"{doc_id}_chk_0", "document_id": doc_id,
        "document_name": name, "user_id": "usr-1", "page_number": page,
        "section": section, "clause_id": "8", "text": text,
    }
...
class TestPhase6Workflow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        p_entry = patch(
            ...
            return_value={"metadata": {"name": "Employment_Contract.pdf",
                                       "indexingStatus": "indexed"}},
        )
        p_key = patch.object(rag_module.settings, "GEMINI_API_KEY",
    ...
    def test_01_doc_scoped_answer(self):
        chunk = _chunk("doc-A", "Employment_Contract.pdf",
                       "The termination notice period is 30 days.")
        with patch.object(rag_module.VectorDatabaseService,
                          "search_similar_chunks", return_value=[chunk]):
            with patch.object(rag_module.VectorDatabaseService,
                              "search_user_chunks") as mock_cross:
                result = self._ask(user_id=self.user, document_id="doc-A",
    ...
    def test_02_cross_doc_fallback(self):
        chunk = _chunk("doc-A", "Employment_Contract.pdf",
                       "The termination notice period is 30 days.")
        with patch.object(rag_module.VectorDatabaseService,
                          "search_similar_chunks", return_value=[]):
            with patch.object(rag_module.VectorDatabaseService,
                              "search_user_chunks", return_value=[chunk]):
                result = self._ask(user_id=self.user, document_id="doc-B",
    ...
    def test_03_no_info_honest(self):
        with patch.object(rag_module.VectorDatabaseService,
                          "search_similar_chunks", return_value=[]):
            with patch.object(rag_module.VectorDatabaseService,
                              "search_user_chunks", return_value=[]):
                with patch.object(rag_module.VectorDatabaseService,
                                  "search_filtered_chunks", return_value=[]):
                    result = self._ask(user_id=self.user, document_id="doc-A",
                                       document_name="Employment_Contract.pdf",
    ...
    def test_04_processing_doc_not_queried(self):
        with patch("app.services.document_service.DocumentManager.get_document_by_id",
                   return_value={"metadata": {"indexingStatus": "indexing"}}):
            with patch.object(rag_module.VectorDatabaseService,
                              "search_similar_chunks") as mock_search:
                result = self._ask(user_id=self.user, document_id="doc-A",
                                   document_name="Employment_Contract.pdf",
                                   question="Anything?")
    ...
    def test_07_general_chat_saves_history(self):
        ...
        with patch.object(GeneralChatService, "_retrieve_user_passages", return_value=[]):
            with patch.object(gc_module.settings, "GEMINI_API_KEY",
                              "your_gemini_api_key_here"):
                resp = asyncio.run(GeneralChatService.ask_question(
                    "What is a lease?", self.user, "conv-g1"))
...
class TestCrossDocIntegrity(unittest.TestCase):
    ...
    def setUp(self):
        ...
        p_load = patch.object(rag_module, "_load_db", side_effect=lambda: self.store)
        p_save = patch.object(rag_module, "_save_db",
                              side_effect=lambda data: self.store.update(data))
        p_embed = patch.object(rag_module.EmbeddingService, "generate_embedding",
        ...
        p_entry = patch(
            "app.services.document_service.DocumentManager.get_document_by_id",
            return_value={"metadata": {"name": "Rental_Agreement.pdf",
                                       "indexingStatus": "indexed"}},
        )
        p_key = patch.object(rag_module.settings, "GEMINI_API_KEY",
                             "your_gemini_api_key_here")
    ...
    def test_cross_doc_answer_attributes_both_documents(self):
        ...
        with patch.object(rag_module.VectorDatabaseService,
                          "search_filtered_chunks", return_value=chunks):
            with patch.object(rag_module.VectorDatabaseService,
                              "search_similar_chunks", return_value=[]):
                with patch.object(rag_module.VectorDatabaseService,
                                  "search_user_chunks", return_value=[]):
                    result = asyncio.run(RAGService.ask_question(
                        user_id=self.user, document_id="doc-B",
                        document_name="Rental_Agreement.pdf",
    ...
    def test_unified_ambiguous_reference_asks_clarification(self):
        result = asyncio.run(RAGService.ask_unified(
            user_id=self.user, question="What does my contract say?",
