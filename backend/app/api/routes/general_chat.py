...
from app.services.ai.general_chat_service import GeneralChatService, GeneralChatResponse
from app.core.logging import logger

router = APIRouter(prefix="/chat", tags=["general-chat"])

class GeneralChatRequest(BaseModel):
    question: str = Field(..., description="General legal question for LegalLens (no document required)")
...
class UnifiedChatRequest(BaseModel):
    ...
    conversation_id: Optional[str] = Field(default=None, description="Optional ongoing conversation ID")
    selected_document_id: Optional[str] = Field(default=None, description="Currently selected document if any (for context)")
    chat_id: Optional[str] = Field(default=None, description="Frontend chat session id (ignored for auth, for logging only)")
...
async def unified_chat(
    ...
    logger.info(f"Unified chat req: user='{current_user['uid'][:8]}...' sel_doc='{body.selected_document_id}' conv='{body.conversation_id}' q='{body.question[:60]}'")
    try:
        ...
        if sel_id:
            ...
            if entry:
                ...
                logger.warning(f"Unified chat: selected doc '{sel_id}' not owned by user '{current_user['uid'][:8]}...'")
                sel_id = None
        ...
        raise HTTPException(status_code=500, detail="LegalLens couldn't answer right now. Please try again.")
...
def get_chat_history(
    ...
    for convo in RAGService.list_user_conversations(user_id):
        if convo.get("document_id") is None:
            messages = RAGService.get_conversation_history(
                user_id, None, convo["conversation_id"])
