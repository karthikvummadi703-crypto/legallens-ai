...
from app.services.ai.schemas import ChecklistItemData, DocumentComparison, FullDocumentAnalysis
...
async def compare_documents(body: CompareRequest, current_user: dict = Depends(get_current_user)):
    """
    ...
    if not body.document_a_id.strip() or not body.document_b_id.strip():
        raise HTTPException(
            status_code=400, detail="Both document_a_id and document_b_id are required."
        )
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to compare documents. Please try again."
        )


@router.get("/{document_id}/checklist", response_model=list[ChecklistItemData])
def get_document_checklist(document_id: str, current_user: dict = Depends(get_current_user)):
    """
...
@router.post(
    "/upload", response_model=DocumentMetadataResponse, status_code=status.HTTP_201_CREATED
)
async def upload_document(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while extracting and saving the document."
        )
...
def get_document_details(document_id: str, current_user: dict = Depends(get_current_user)):
    """
    ...
    safe_entry = {k: v for k, v in entry.items() if k not in ("saved_path",)}
    return safe_entry


@router.post("/{document_id}/analyze", response_model=FullDocumentAnalysis)
async def analyze_document(document_id: str, current_user: dict = Depends(get_current_user)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to run document analysis. Please try again."
        )


@router.get("/{document_id}/analysis", response_model=FullDocumentAnalysis)
def get_document_analysis(document_id: str, current_user: dict = Depends(get_current_user)):
    """
    ...
    if not analysis:
        raise HTTPException(
            status_code=404, detail=f"Analysis for document '{document_id}' has not been run yet."
        )
    return analysis
