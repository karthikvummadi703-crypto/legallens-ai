...
from app.services.ai.prompts import RAG_USER_PROMPT_TEMPLATE, SYSTEM_RAG_PROMPT
...
class SourceCitation(BaseModel):
    ...
    clause_id: str | None = Field(default="")
...
class RAGService:
    ...
    @classmethod
    def _detect_referenced_docs(
        cls, question: str, library_map: dict, selected_document_id: str | None = None
    ) -> dict:
        """
        ...
        if has_cross_signal and len(matches) >= 2:
            return {
                "mode": "multi",
                "doc_ids": [m[0] for m in matches],
                "ambiguous": False,
                "reason": "cross_signal_multi",
            }
        if has_cross_signal and len(matches) == 1:
            if any(kw in q_lower for kw in ["compare", "both", "between", "difference"]):
                return {
                    "mode": "all",
                    "doc_ids": list(library_map.keys()),
                    "ambiguous": False,
                    "reason": "compare_single_match_fallback_all",
                }
            return {
                "mode": "single_referenced",
                "doc_ids": [matches[0][0]],
                "ambiguous": False,
                "reason": "single_with_cross_signal",
            }
        if has_cross_signal:
            return {
                "mode": "all",
        ...
        if len(matches) == 1 and len(library_map) > 1 and matches[0][2] < 1.0:
            ...
            if norm not in q_lower:
                weak_hits = [
                    t
                    for t in norm.split()
                    if len(t) > 2 and t in q_lower and t in cls.GENERIC_DOC_TOKENS
                ]
                non_weak_hits = [
                    t
                    for t in norm.split()
                    if len(t) > 2 and t in q_lower and t not in cls.GENERIC_DOC_TOKENS
                ]
        ...
        if len(matches) >= 2:
            return {
                ...
                "doc_ids": [m[0] for m in matches],
                "ambiguous": False,
                "reason": "multi_reference",
            }
        # No explicit reference: use selected doc
        return {
            "mode": "specific",
            "doc_ids": [selected_document_id] if selected_document_id else [],
            "ambiguous": False,
            "reason": "no_reference_specific",
        }
    ...
    async def ask_question(
        ...
        logger.info(
            f"RAG Q&A: user='{user_id[:8]}...' doc='{document_id}' conv='{conversation_id}' q='{question[:80]}'"
        )
        ...
        indexing_status = (entry.get("metadata", {}) if entry else {}).get(
        ...
        if indexing_status not in ("indexed",):
            ...
            cls._save_message_pair(
                user_id, document_id, conversation_id, question, result.answer, []
            )
            return result
        ...
        library_text = (
            ...
            if library_names
            else "Available user documents: (none besides the selected document)"
        )
        ...
        logger.info(
            f"Dev RAG Scope detect: mode='{scope['mode']}' doc_ids={scope['doc_ids']} reason='{scope['reason']}' q='{question[:60]}'"
        )
        ...
        if scope.get("ambiguous"):
            ...
            result = RAGAnswerResponse(
                ...
                confidence="low",
                followup_questions=[
                    f"Summarize {library_map[did]}" for did in scope["doc_ids"][:2]
                ],
            )
            cls._save_message_pair(
                user_id, document_id, conversation_id, question, result.answer, []
        ...
        if scope["mode"] == "single_referenced":
            ...
            if ref_id != document_id:
                ...
                ref_status = (ref_entry.get("metadata", {}) if ref_entry else {}).get(
                    "indexingStatus", "indexed"
                )
                ...
                if not retrieved_chunks:
                    cross_chunks = VectorDatabaseService.search_filtered_chunks(
                        user_id=user_id,
                        document_ids=scope["doc_ids"],
                        query_vector=query_vector,
                        top_k=cls.CROSS_DOC_TOP_K,
                    )
                    if cross_chunks:
                        retrieved_chunks = cross_chunks
                logger.info(
                    f"RAG scope='{retrieval_scope}' single_referenced doc={ref_id} chunks={len(retrieved_chunks)}"
                )
            else:
            ...
            if len(target_ids) == 1:
                ...
                retrieval_scope = "cross-document"
            logger.info(
                f"RAG scope='{retrieval_scope}' multi/all docs={target_ids} chunks={len(retrieved_chunks)}"
            )
        else:
            ...
            for idx, chunk in enumerate(retrieved_chunks):
                logger.info(
                    f"Dev RAG Retrieval #{idx + 1}: Score={chunk.get('score', 0):.4f} | Page={chunk.get('page_number')} | Sec={chunk.get('section')}"
                )
        ...
        if scope["mode"] != "specific":
            for idx, chunk in enumerate(retrieved_chunks):
                logger.info(
                    f"Dev RAG Retrieval #{idx + 1}: Score={chunk.get('score', 0):.4f} | Doc={chunk.get('document_id')} | Page={chunk.get('page_number')} | Sec={chunk.get('section')}"
                )
        ...
        if not retrieved_chunks:
            ...
            result = RAGAnswerResponse(
                ...
                followup_questions=general.followup_questions
                or [
            ...
            for api_key in gemini_key_manager.iter_keys():
                try:
                    ...
                    if response and response.text:
                        parsed = cls._parse_rag_json(
                            response.text, conversation_id, retrieved_chunks
                        )
                        if parsed:
                            ...
                            cls._save_message_pair(
                                ...
                                question,
                                parsed.answer,
                                [s.model_dump() for s in parsed.sources],
                                referenced=list(
                                    {
                                        c.get("document_id", "")
                                        for c in retrieved_chunks
                    ...
                    logger.error(
                        f"Gemini RAG SDK error ({e}). Using grounded snippet synthesis engine."
                    )
                    break
        ...
        cls._save_message_pair(
            ...
            [s.model_dump() for s in result.sources],
            referenced=list(
                {c.get("document_id", "") for c in retrieved_chunks if c.get("document_id")}
            ),
        )
        return result
    ...
    async def ask_unified(
        ...
        if selected_document_id:
            ...
            if not doc_name:
                try:
                    ...
                    doc_name = (entry.get("metadata", {}) if entry else {}).get(
        ...
        if not conversation_id:
            conversation_id = f"conv-{uuid.uuid4().hex[:8]}"
        logger.info(
            f"Unified RAG (library-wide): user='{user_id[:8]}...' conv='{conversation_id}' q='{question[:80]}'"
        )
        library_map = cls._user_library_map(user_id)
        library_names = list(library_map.values())
        library_text = (
            "Available user documents:\n" + "\n".join(f"- {n}" for n in library_names)
            if library_names
            else "Available user documents: (none)"
        )
        scope = cls._detect_referenced_docs(question, library_map, selected_document_id=None)
        logger.info(
            f"Unified scope: mode='{scope['mode']}' doc_ids={scope['doc_ids']} reason='{scope['reason']}'"
        )
        if scope.get("ambiguous"):
            doc_list = " or ".join([f"'{library_map[did]}'" for did in scope["doc_ids"][:3]])
            answer_text = f"Which document do you mean — {doc_list}?"
        ...
        if not retrieved_chunks:
            ...
            if not library_map:
                return await GeneralChatService.ask_question(question, user_id, conversation_id)
        ...
        if len({c.get("document_id") for c in retrieved_chunks}) > 1:
            result.answer = (
                f"Based on your document library (including '{top_doc}'):\n\n" + result.answer
            )
        else:
    ...
    def get_conversation_history(
        cls, user_id: str, document_id: str | None, conversation_id: str | None = None
    ) -> list[dict]:
        db = _load_db()
        ...
        for doc_id in document_ids[:3]:
            try:
                ...
                for r in (analysis.potential_risks or [])[:3]:
                    lines.append(f"- Risk ({r.severity}): {r.title} [Page {r.page}, {r.section}].")
                ...
                if (
                    term
                    and getattr(term, "summary", "")
                    and "no termination provision" not in term.summary.lower()
                ):
                    lines.append(
                        f"- Termination: {term.summary[:160]} [Page {term.page}, {term.section}]."
                    )
                if len(lines) > 1:
                    blocks.append("\n".join(lines))
            except Exception as e:
    ...
    def _small_talk_reply(
        cls, question: str, conversation_id: str, context_name: str | None = None
    ) -> RAGAnswerResponse:
        ...
        if any(p in q for p in ("thank", "thanks")):
            ...
            answer = (
                "Goodbye! Your documents and chat history are saved here whenever you need them."
            )
            ...
            doc_hint = (
                ...
                if context_name
                else " Upload a contract and I'll explain it in plain language — or just ask me "
                "any general legal question right away."
            )
            answer = f"Hello! I'm LegalLens AI, your legal document assistant.{doc_hint}"
    ...
    def _save_message_pair(
        ...
        if existing and (
            existing.get("user_id") != user_id or existing.get("document_id") != document_id
        ):
            raise ValueError("Conversation is not associated with this user/document.")

        conv_entry = db["conversations"].get(
            conversation_id,
            {
                "conversation_id": conversation_id,
                "user_id": user_id,
        ...
        conv_entry["messages"].append(
            {"role": "assistant", "content": answer, "sources": sources, "created_at": ts}
        )
    ...
    @classmethod
    def _parse_rag_json(
        cls, text: str, conversation_id: str, retrieved_chunks: list
    ...
    @classmethod
    def _generate_grounded_fallback_rag(
        cls, question: str, retrieved_chunks: list, conversation_id: str
    ) -> RAGAnswerResponse:
        """
        ...
        for chunk in retrieved_chunks[:3]:
            ...
            for sentence in picked:
                ...
                if sentence and sentence not in quotes:
                    ...
                    if chunk is not top:
                        c_doc = (
                            chunk.get("document_name") or chunk.get("document_id") or ""
                        ).strip()
                        label = (
