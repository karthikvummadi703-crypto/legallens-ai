...
from app.services.ai.prompts import ANALYSIS_USER_PROMPT_TEMPLATE, SYSTEM_LEGAL_ANALYSIS_PROMPT
...
class GeminiAnalysisService:
    ...
    async def analyze_document(
        cls, extracted_doc: ExtractedDocument, user_id: str
    ) -> FullDocumentAnalysis:
        ...
        if gemini_key_manager.has_keys():
            for api_key in gemini_key_manager.iter_keys():
                try:
                    logger.info(
                        f"Invoking Gemini API ({settings.GEMINI_MODEL}) for document {extracted_doc.document_id}..."
                    )
                    analysis = await cls._call_gemini_api(extracted_doc, user_id, api_key)
                    if analysis:
                        gemini_key_manager.report_success(api_key)
                        logger.info(
                            f"Gemini API document analysis successfully generated and validated for {extracted_doc.document_id}."
                        )
                        return analysis
                    # Empty/soft failure: try the next key before falling back.
                    logger.warning(
                        "Gemini API returned no usable analysis; trying next key if available."
                    )
                    ...
                    logger.error(
                        f"Gemini API call or validation failed, using grounded fallback analysis engine: {e}"
                    )
                    break
            else:
                ...
                logger.error(
                    "All Gemini API keys exhausted or failed; using grounded fallback analysis engine."
                )
        else:
            logger.info(
                "No active GEMINI_API_KEY configured. Utilizing grounded AI analysis engine."
            )
    ...
    async def _call_gemini_api(
        cls, extracted_doc: ExtractedDocument, user_id: str, api_key: str
    ) -> FullDocumentAnalysis | None:
        try:
            ...
            prompt = ANALYSIS_USER_PROMPT_TEMPLATE.format(
                ...
                extracted_text=extracted_doc.full_text[
                    :60000
                ],  # Increased from 35K — Gemini Flash supports 1M+ tokens
            )
    ...
    def _parse_and_validate_json(
        cls, text: str, extracted_doc: ExtractedDocument, user_id: str
    ) -> FullDocumentAnalysis | None:
    ...
    @classmethod
    def _generate_grounded_fallback_analysis(
        cls, extracted_doc: ExtractedDocument, user_id: str
    ) -> FullDocumentAnalysis:
        """
        ...
        if extracted_doc.clauses:
            key_points.append(
                f"Extraction identified {len(extracted_doc.clauses)} numbered clause(s) in the document."
            )
        ...
        if "lease" in filename_lower or "rental" in filename_lower:
            ...
            k in lowered_terms
            for k in ("contract", "agreement", "policy", "terms", "clause", "hereby", "whereas")
        ):
        ...
        if not key_points:
            key_points.append(
                f"Document contains {max_page} page(s) and {len(extracted_doc.sections)} detected section(s)."
            )
        ...
        for c in extracted_doc.clauses[:10]:
            ...
            if any(k in text_lower for k in obligation_keywords):
                _add_obligation(
                    ...
                    (c.text[:200] if c.text else (c.title or "Obligation")),
                    c.page,
        ...
        termination_text = next(
            (
                c.text
                for c in extracted_doc.clauses
                if "terminat" in (c.title or "").lower() or "terminat" in (c.text or "").lower()
            ),
            None,
        )
        if termination_text:
        ...
        renewal_text = next(
            (
                c.text
                for c in extracted_doc.clauses
                if "renew" in (c.title or "").lower() or "renew" in (c.text or "").lower()
            ),
            None,
        )
        if renewal_text:
            ...
            renewal = RenewalAnalysis(
                has_renewal=True,
                automatic_renewal="automatic" in renewal_text.lower()
                or "automatically" in renewal_text.lower(),
