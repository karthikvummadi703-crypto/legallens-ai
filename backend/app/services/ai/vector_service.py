...
def _memory_snapshot_path() -> str:
    ...
    try:
        ...
        return os.path.join(os.path.dirname(__file__), "..", "..", "..", "data", "vector_memory_snapshot.json")
    ...
    try:
        if is_cloud_mode():
            ...
            if data:
                ...
                logger.info(f"Memory Vector Store: Restored {len(data)} chunk vectors from Firebase Realtime Database.")
            return
        import json
        ...
        if os.path.exists(path):
            ...
            if isinstance(data, dict):
                _in_memory_store.update(data)
                logger.info(f"Memory Vector Store: Restored {len(data)} chunk vectors from disk snapshot.")
...
def _get_qdrant_client():
    ...
    if _qdrant_client is None:
        if is_cloud_mode():
            ...
            logger.info("Qdrant disabled in cloud (serverless) mode; using Firebase-backed memory vector store.")
        ...
        try:
            ...
            if COLLECTION_NAME not in collections:
                ...
                logger.info(f"Qdrant collection '{COLLECTION_NAME}' created (Vector size: {VECTOR_SIZE}).")
            ...
            logger.warning(f"Qdrant client initialization notice ({e}). Using robust in-memory vector store.")
            _qdrant_client = False
    return _qdrant_client
...
class VectorDatabaseService:
    ...
    def upsert_document_chunks(
        ...
        if client:
            try:
                ...
                logger.info(f"Qdrant: Upserted {len(points)} vectors for document {document_id} (User: {user_id}).")
                return True
            except Exception as e:
        ...
        logger.info(f"Memory Vector Store: Indexed {len(chunks)} chunks for document {document_id}.")
        return True
    ...
    def search_similar_chunks(
        ...
        if client:
            try:
                ...
                logger.info(f"Qdrant Search: Retrieved {len(results)} chunks for doc {document_id} (User: {user_id}).")
                return results
            except Exception as e:
        ...
        logger.info(f"In-Memory Vector Search: Retrieved {len(top_results)} chunks for doc {document_id}.")
        return top_results
    ...
    def search_user_chunks(
        ...
        if client:
            try:
                ...
                logger.info(f"Qdrant Cross-Doc Search: Retrieved {len(results)} chunks (User: {user_id}).")
                return results
            except Exception as e:
                logger.error(f"Qdrant cross-doc search error ({e}). Using in-memory fallback search.")
        ...
        logger.info(f"In-Memory Cross-Doc Search: Retrieved {len(top_results)} chunks (User: {user_id}).")
        return top_results
    ...
    def search_filtered_chunks(
        ...
        if client:
            try:
                ...
                try:
                    doc_condition = FieldCondition(key="document_id", match=MatchAny(any=document_ids))
                except Exception:
                    # Fallback if MatchAny signature differs
                    doc_condition = FieldCondition(key="document_id", match=MatchValue(value=document_ids[0]))
                search_filter = Filter(
                    must=[
                        FieldCondition(key="user_id", match=MatchValue(value=user_id)),
                        doc_condition
                ...
                if not results:
                    ...
                    results = [c for c in all_chunks if c.get("document_id") in document_ids][:top_k]
                logger.info(f"Qdrant Filtered Search: Retrieved {len(results)} chunks for docs {document_ids} (User: {user_id}).")
                return results
            except Exception as e:
                logger.error(f"Qdrant filtered search error ({e}). Using in-memory fallback search.")
        ...
        if not scored_chunks:
            ...
            if broad:
                logger.info(f"In-Memory Filtered Search: Broadened to user-wide, kept {len(broad)} chunks for docs {document_ids}.")
                return broad
        scored_chunks.sort(key=lambda x: x[0], reverse=True)
        top_results = []
        for score, payload in scored_chunks[:top_k]:
        ...
        logger.info(f"In-Memory Filtered Search: Retrieved {len(top_results)} chunks for docs {document_ids}.")
        return top_results
    ...
    def delete_document_vectors(cls, user_id: str, document_id: str) -> bool:
        ...
        keys_to_delete = [
            k for k, v in _in_memory_store.items() 
            if v["payload"].get("user_id") == user_id and v["payload"].get("document_id") == document_id
        ]
    ...
    def count_document_vectors(cls, user_id: str, document_id: str) -> int:
        ...
        return sum(
            1 for v in _in_memory_store.values()
    ...
    def delete_user_vectors(cls, user_id: str) -> bool:
        ...
        keys_to_delete = [k for k, v in _in_memory_store.items() if v["payload"].get("user_id") == user_id]
        for k in keys_to_delete:
            del _in_memory_store[k]
        if keys_to_delete:
            _persist_memory_snapshot()
