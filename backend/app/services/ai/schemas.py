...
class ExecutiveSummary(BaseModel):
    summary: str = Field(
        ...,
        description="Simple-language summary understandable to a non-lawyer without unnecessary legal jargon.",
    )
    key_points: list[str] = Field(
        default_factory=list,
        description="3 to 5 key takeaway bullet points supported by the document.",
    )
...
class ImportantClause(BaseModel):
    clause_id: str = Field(
        default="",
        description="Clause identifier e.g. 8.2 or Section 4 present in extracted document",
    )
...
class PotentialRisk(BaseModel):
    ...
    category: str = Field(
        ...,
        description="Risk category: Financial, Termination, Liability, Renewal, Obligations, Privacy, Confidentiality, Dispute Resolution, Restrictive clauses, Ambiguity, or Unusual terms",
    )
    severity: Literal["high", "medium", "low", "informational"] = Field(default="medium")
    explanation: str = Field(
        ...,
        description="Cautious explanation of the potential risk (e.g. 'This clause may create financial risk...')",
    )
    why_it_matters: str = Field(
        ..., description="Practical business or personal impact for the user"
    )
    ...
    clause_id: str | None = Field(default="", description="Referenced clause ID")
    source_text: str = Field(default="", description="Quoted text from document")
    suggested_question: str = Field(
        default="", description="Suggested question to clarify this risk"
    )
...
class ObligationItem(BaseModel):
    party: str = Field(
        ...,
        description="Party bound by obligation (e.g. Tenant, Employee, Company, Service Provider)",
    )
    obligation: str = Field(
        ..., description="Explicit required action or duty explicitly present in document"
    )
    deadline: str = Field(default="As specified in document", description="Timeframe or deadline")
    consequence: str = Field(
        default="Standard default terms", description="Explicit consequence of failure to perform"
    )
...
class KeyDateItem(BaseModel):
    event: str = Field(
        ...,
        description="Event name (e.g. Start date, End date, Renewal date, Notice period, Payment deadline, Termination notice, Delivery deadline)",
    )
    date_or_period: str = Field(
        ..., description="Specific date or time period mentioned in document"
    )
    importance: Literal["high", "medium", "low"] = Field(default="medium")
...
class PaymentDetails(BaseModel):
    item: str = Field(
        ...,
        description="Payment item name e.g. Base Rent, Monthly Retainer, Security Deposit, Late Fee",
    )
    amount: str = Field(..., description="Amount preserving original currency and wording")
    frequency: str = Field(
        default="One-time", description="Payment frequency (e.g. Monthly, Annually, Lump-sum)"
    )
    ...
    deposits: str | None = Field(default="None specified", description="Deposit terms")
    refund_conditions: str | None = Field(
        default="None specified", description="Conditions for refunds"
    )
    additional_charges: str | None = Field(
        default="None specified", description="Additional fees or charges"
    )
    page: int = Field(default=1)
    section: str = Field(default="General")
...
class TerminationAnalysis(BaseModel):
    ...
    early_termination: str | None = Field(
        default="Not specified", description="Early termination rights"
    )
    penalties: str = Field(
        default="None specified", description="Early termination penalties or fees"
    )
    consequences: str | None = Field(
        default="Standard terms", description="Consequences of termination"
    )
...
class RenewalAnalysis(BaseModel):
    ...
    renewal_period: str | None = Field(default="N/A", description="Duration of renewal term")
    ...
    consequences: str | None = Field(default="N/A", description="Consequences of renewal")
    summary: str = Field(
        default="No renewal provision was identified in the document.",
        description="Explanation of renewal terms or 'No renewal provision was identified in the document.'",
    )
    page: int = Field(default=1)
...
class LawyerQuestionItem(BaseModel):
    question: str = Field(
        ..., description="Specific question for a lawyer referencing document content"
    )
    context: str = Field(
        ..., description="Rationale for asking this question based on document text"
    )
    page: int = Field(default=1)
    section: str = Field(default="General")
...
class AttentionScoreDetails(BaseModel):
    ...
    label: str = Field(
        ..., description="'Requires Immediate Attention' | 'Requires Review' | 'Standard Terms'"
    )
    reasoning: str = Field(
        ..., description="Transparent breakdown of derived score based on risk count & severity"
    )
...
class FullDocumentAnalysis(BaseModel):
    ...
    disclaimer: str = Field(
        default="LegalLens provides informational assistance and does not replace professional legal advice."
    )
...
class ComparisonDifference(BaseModel):
    ...
    ai_explanation: str = Field(
        default="", description="Plain-language explanation of why this difference matters"
    )
...
class ChecklistItemData(BaseModel):
    ...
    section: str = Field(
        default="General",
        description="Checklist grouping: Payments, Term, Termination, Obligations, Liability, Dispute Resolution, Renewal",
    )
    title: str = Field(..., description="Short verification task title")
    explanation: str = Field(
        default="", description="Why this item deserves verification before signing"
    )
    page: int = Field(default=1)
    section_ref: str = Field(
        default="General", description="Source section heading in the document"
    )
    attention_level: Literal["high", "medium", "low", "informational"] = Field(default="medium")
