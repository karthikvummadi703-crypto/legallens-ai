...
def validate_uploaded_file(file: UploadFile, content_bytes: bytes) -> str:
    ...
    if ext == ".pdf" and not content_bytes.lstrip()[:5].startswith(b"%PDF"):
        raise HTTPException(status_code=400, detail="File extension is .pdf but content does not appear to be a valid PDF.")
    
    if ext == ".docx" and not content_bytes[:4] == b"PK\x03\x04":
        raise HTTPException(status_code=400, detail="File extension is .docx but content does not appear to be a valid DOCX archive.")
